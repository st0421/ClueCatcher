...
def iqa_load_face(record: pd.Series, root: str, size: int, scale: str, transformer: A.BasicTransform) -> torch.Tensor:
    ...
    name = str(record.name).split('\\')

    if name[0].startswith('Celeb'):
        df = pd.read_pickle(root+'/'+name[0]+".pkl")
        df_real = pd.read_pickle(root+'/Celeb-real.pkl')
        input_name = name[1]+'_'+name[2]
        real_name_list = name[1].split('_')
        real_name = real_name_list[0]+'_'+real_name_list[2]
        iqa_real_m = np.mean(df_real[real_name])
        
        iqa = np.float64(df[input_name])
        iqa = abs(iqa-iqa_real_m)
    else: #FaceForensics++
        df = pd.read_pickle(root+'/'+name[1]+".pkl")
        df_real = pd.read_pickle(root+'/youtube.pkl')
        real_name = name[-2].split('.')[0].split('_')[0]+'.mp4'

        if name[1] =='youtube':
            iqa = np.float64(df_real[real_name][0])
        else:
            key=name[-2]+"_"+name[-1]
            iqa = np.float64(df[key][0])
        iqa_real_m = np.mean(df_real[real_name])
        iqa = abs(iqa-iqa_real_m)
    ...
    iqa_arr = np.array(iqa)
    return face, iqa_arr

class IQAFrameFaceIterableDataset(IterableDataset):
    ...
    def __init__(self,
        ...
        self.seed0 = int(seed) if seed is not None else np.random.choice(2 ** 30)
    ...
    def _get_face(self, item: pd.Index):

        record = self.dfs[item[0]].loc[item[1]]
        face, iqa_arr = iqa_load_face(record=record,
        ...
        if self.output_idx:
            return face, iqa_arr, label, record.name
        else:
            return face, iqa_arr, label
...
class IQAFrameFaceDatasetTest(Dataset):
