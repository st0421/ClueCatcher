...
def get_split_df(df: pd.DataFrame, dataset: str, split: str) -> pd.DataFrame:
    ...
    elif dataset.startswith('ff-c23-cross'):
        crf = dataset.split('-')[1]
        random_youtube_videos = np.random.permutation(df[(df['source'] == 'youtube') & (df['quality'] == crf)]['video'].unique())
        train_orig = random_youtube_videos
        cross_train = dataset.split('-')[3]
        if cross_train == 'df':
            cross_train = 'Deepfakes'
        elif cross_train == 'f2f':
            cross_train = 'Face2Face'
        elif cross_train == 'fs':
            cross_train = 'FaceSwap'
        elif cross_train == 'nt':
            cross_train = 'NeuralTextures'
        if split == 'train':
            split_df = pd.concat((df[df['original'].isin(train_orig)&(df['source'] == (cross_train or 'youtube'))], df[df['video'].isin(train_orig)]), axis=0)
        elif split == 'val':
            split_df = pd.concat((df[df['original'].isin(train_orig)&(df['source'] != cross_train)], df[df['video'].isin(train_orig)]), axis=0)
        elif split == 'test':
            split_df = pd.concat((df[df['original'].isin(train_orig)&(df['source'] == cross_train)], df[df['video'].isin(train_orig)]), axis=0)
        else:
            raise NotImplementedError('Unknown split: {}'.format(split))
