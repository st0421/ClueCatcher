...
from isplutils.data import IQAFrameFaceIterableDataset, load_face
...
def main():
    ...
    parser.add_argument('--workers', type=int, help='Num workers for data loaders', default=1)
    ...
    if iteration == 0:
        ...
        dum = torch.randn((1), device=device)
        dummy = dummy.to(device)
        dum = dum.to(device)
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            tb.add_graph(net, [dummy,dum], verbose=False)
    ...
    train_dataset = IQAFrameFaceIterableDataset(roots=train_roots,
                                             dfs=train_dfs,
                                             scale=face_policy,
    ...
    val_dataset = IQAFrameFaceIterableDataset(roots=val_roots,
                                           dfs=val_dfs,
                                           scale=face_policy,
    ...
    while not stop:
        ...
        for train_batch in tqdm(train_loader, desc='Epoch {:03d}'.format(epoch), leave=False,
            ...
            net.train()
            batch_data, iqa_arr, batch_labels = train_batch
            train_batch_num = len(batch_labels)
            train_num += train_batch_num
            train_labels_list.append(batch_labels.numpy().flatten())

            train_batch_loss, train_batch_pred = batch_forward(net, device, criterion, batch_data,  iqa_arr.float(), batch_labels)
...
def batch_forward(net: nn.Module, device: torch.device, criterion, data: torch.Tensor,  iqa_arr, labels: torch.Tensor):
    data = data.to(device)
    iqa_arr = iqa_arr.to(device)
    labels = labels.to(device)
    out, feats = net(data,iqa_arr)
...
def validation_routine(net, device, val_loader, criterion, tb, iteration, tag: str, loader_len_norm: int = None):
    ...
    for val_data in tqdm(val_loader, desc='Validation', leave=False, total=len(val_loader) // loader_len_norm):
        batch_data, iqa_arr,  batch_labels  = val_data

        val_batch_num = len(batch_labels)
        labels_list.append(batch_labels.flatten())
        with torch.no_grad():
            val_batch_loss, val_batch_pred = batch_forward(net, device, criterion, batch_data, iqa_arr.float(), batch_labels)
