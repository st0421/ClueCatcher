...
from isplutils.data import IQAFrameFaceDatasetTest
...
def process_dataset(df: pd.DataFrame,
    ...
    dataset = IQAFrameFaceDatasetTest(
        root=root,
        df=df,
        size=patch_size,
        scale=face_policy,
    ...
    with torch.no_grad():
        ...
        for batch_data in tqdm(loader):
            ...
            iqa_arr = batch_data[1]
            batch_images = batch_images.to(device)
            iqa_arr = iqa_arr.to(device)
            batch_labels = batch_data[2].to(device)
            batch_samples = len(batch_images)
            batch_out, feats = net(batch_images,iqa_arr.float())
